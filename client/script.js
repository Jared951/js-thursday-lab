...
// PROBLEM 2
/*
    Create a function that changes sayHelloButton's background color to black 
    and its text color to white (you can use the .style object or create a CSS class 
...
/*
    ...
    Attach another listener that fires your second function 
    when the mouseout event occurs on the button
*/
...
/*
    ...
    inside repeatMyParam function below  make get request to 
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/js-lab-jared`)
    .then((res) => {
        console.log(res.data)
    })
    .catch((err) => {
    ...
    let repeatOne = document.getElementById('repeat-text')
    repeatOne.textContent = res.data
...
const getRequest = () => {
    axios.get(`http://localhost:3000/query-test/jared-lab-2`)
}

document.addEventListener('click', '#query-button')
